...
| `0x100` | bytes0–1, 13-bit Motorola field (`byte0[4:0]`, then byte1) | strong physical candidate: `raw × 0.125 - 512 Nm` | **engine-torque-related quantity, semantic stage unresolved**. The packed scale is strongly supported by the loaded range and a -106 ms comparison to signed PCM DID `06DA`: `DID Nm = 1.00201 × candidate Nm + 7.118`, R² 0.98257, 10.89 Nm affine RMSE. The persistent stage offset and mode-dependent outliers mean this is not yet interchangeable with Alfa's `Current engine torque`; do not publish it or derive power from it. | ignition ON / running | strong packed-field/scale candidate, exact torque stage unresolved; [idle](../projects/ecu_mapping/findings/promaster_2022/2026-07-26_pcm_plots_idle_mapping.md), [loaded drive](../projects/ecu_mapping/findings/promaster_2022/2026-07-27_pcm_plots_loaded_drive_mapping.md) |
| `0x100` | bytes3–4 BE u16, upper 11 bits | `(raw >> 5) - 500 Nm` | **TCM target crankshaft torque**. Exact Alfa gauge/DID association is `101B`. This is a commanded target, not measured output; telemetry labels it explicitly and converts Nm to lb-ft. | ignition ON / running | **observed Alfa scale; receive-only telemetry allowlisted**, 169 samples, R² 0.99998794 and 0.104 DID-raw-count timing RMSE; [TCM loaded-drive evidence](../projects/ecu_mapping/findings/promaster_2022/2026-07-27_tcm_plots_loaded_drive_mapping.md) |
...
| `0x417` | bytes2–3 BE u16 and adjacent packed views | scaling withdrawn | **thermal/state candidate, not gearbox-oil temperature.** The development leg covaried with both TCM `04FE` oil and `0301` chip temperature, but two independent blind drives failed the oil hypothesis: the exact bytes2–3 view fell below the first leg's reported top 100 and reached only R² 0.44824 with a negative slope on the second. The first blind leg retained a materially stronger chip-temperature association (R² 0.89384). Its prior `raw / 64 - 2 °C` decode does not transfer across legs. | ignition ON / running | gearbox-oil identity rejected by current independent evidence; chip-temperature association remains candidate-only pending the second comparator and stable scaling; [packed-field benchmark](../projects/ecu_mapping/findings/promaster_2022/2026-07-28_signal_field_engine_benchmark.md) |
...
| `0x1F7` | bytes4–5 BE u16 | `raw / 2 rpm` | **transmission turbine/input speed**, exact-linked to TCM DID `2102` and its labeled Alfa gauge | ignition ON / running | **observed Alfa scale; receive-only telemetry allowlisted**, 248 samples, R² 0.99999024; same evidence |
