...
| `0x100` | byte2 / broader frame | wrap/mode unresolved; idle-only byte2 fit looked like `byte - 244 Nm` | **current-engine-torque-related candidate** tracking signed PCM DID `06DA` (DID raw x 0.04 Nm). The loaded drive disproved a simple byte2 decode: the best `0x100` view was u16be bytes0–1 at only R² 0.9658 and 15.25 Nm display-domain RMSE. Do not publish it or derive power from it. | ignition ON / running | strong related-signal evidence, physical decode unresolved; [idle](../projects/ecu_mapping/findings/promaster_2022/2026-07-26_pcm_plots_idle_mapping.md), [loaded drive](../projects/ecu_mapping/findings/promaster_2022/2026-07-27_pcm_plots_loaded_drive_mapping.md) |
| `0x1F4` | bytes0–1 BE u16 | nonlinear/mode unresolved | strongest loaded-drive **current-engine-torque-related candidate** against PCM DID `06DA`; it outranked `0x100` but does not have an exact affine physical decode. | ignition ON / running | candidate only, 1,351/1,355 samples, R² 0.9831 and 10.72 Nm display-domain RMSE; same loaded-drive evidence |
